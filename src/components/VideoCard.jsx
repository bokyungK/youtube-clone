...
export default function VideoCard({ video, type }) {
    ...
    return (
        <li className={`flex ${type ? 'flex-col mb-[15px]' : 'mb-[5px]'}`}>
            <img src={thumbnails.medium.url} alt="thumbnail image" className={`${type ? 'w-full mb-[10px]' : 'mr-[10px] w-[132.5px] xs:w-[200px] md:w-[132.5px]'}`} />
            <div className={`text-white ${type ? 'w-full' : 'w-[50%] flex-1'}`}>
                <p className={`mb-[5px] font-semibold text-ellipsis line-clamp-2 break-all text-sm ${type && 'w-full text-sm mb-[5px]'}`}>{title}</p>
